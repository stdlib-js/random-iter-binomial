...
function iterator( n, p, options ) {
	...
	if ( !isPositiveInteger( n ) ) {
		throw new TypeError( format( '0Pa4o', n ) );
	}
	if ( !isProbability( p ) ) {
		throw new TypeError( format( '0Pa7L', p ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Pa2h', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Pa35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
